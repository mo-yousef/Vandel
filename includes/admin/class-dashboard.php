...
/**
    ...
    private function renderClientsTab() {
        ...
        $this->displayClientsStatusMessage();
                ...
                <div class="vandel-card-body">
                    ...
                    <div class="vandel-filters-toolbar">
                        <form method="get" action="<?php echo admin_url('admin.php'); ?>" class="vandel-filter-form">
                            ...
                            <div class="vandel-search-field">
                                <input type="text" name="s" placeholder="<?php _e('Search clients...', 'vandel-booking'); ?>" value="<?php echo isset($_GET['s']) ? esc_attr($_GET['s']) : ''; ?>">
                                <button type="submit" class="vandel-search-button">
                                    <span class="dashicons dashicons-search"></span>
                                </button>
                            </div>
                    ...
                    <form method="post" id="vandel-clients-form">
                        ...
                        <?php else: ?>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" class="manage-column column-name">
                                            <a href="<?php echo add_query_arg(['orderby' => 'name', 'order' => (isset($_GET['orderby']) && $_GET['orderby'] === 'name' && isset($_GET['order']) && $_GET['order'] === 'ASC') ? 'DESC' : 'ASC']); ?>">
                                                <?php _e('Name', 'vandel-booking'); ?>
                                                <?php if (isset($_GET['orderby']) && $_GET['orderby'] === 'name'): ?>
                                                    <span class="sorting-indicator"></span>
                                                <?php endif; ?>
                                            </a>
                                        </th>
                                        <th scope="col" class="manage-column column-email">
                                            <a href="<?php echo add_query_arg(['orderby' => 'email', 'order' => (isset($_GET['orderby']) && $_GET['orderby'] === 'email' && isset($_GET['order']) && $_GET['order'] === 'ASC') ? 'DESC' : 'ASC']); ?>">
                                                <?php _e('Email', 'vandel-booking'); ?>
                                                <?php if (isset($_GET['orderby']) && $_GET['orderby'] === 'email'): ?>
                                                    <span class="sorting-indicator"></span>
                                                <?php endif; ?>
                                            </a>
                                        </th>
                                        <th scope="col" class="manage-column column-phone"><?php _e('Phone', 'vandel-booking'); ?></th>
                                        <th scope="col" class="manage-column column-bookings">
                                            <a href="<?php echo add_query_arg(['orderby' => 'bookings_count', 'order' => (isset($_GET['orderby']) && $_GET['orderby'] === 'bookings_count' && isset($_GET['order']) && $_GET['order'] === 'ASC') ? 'DESC' : 'ASC']); ?>">
                                                <?php _e('Bookings', 'vandel-booking'); ?>
                                                <?php if (isset($_GET['orderby']) && $_GET['orderby'] === 'bookings_count'): ?>
                                                    <span class="sorting-indicator"></span>
                                                <?php endif; ?>
                                            </a>
                                        </th>
                                        <th scope="col" class="manage-column column-total-spent">
                                            <a href="<?php echo add_query_arg(['orderby' => 'total_spent', 'order' => (isset($_GET['orderby']) && $_GET['orderby'] === 'total_spent' && isset($_GET['order']) && $_GET['order'] === 'ASC') ? 'DESC' : 'ASC']); ?>">
                                                <?php _e('Total Spent', 'vandel-booking'); ?>
                                                <?php if (isset($_GET['orderby']) && $_GET['orderby'] === 'total_spent'): ?>
                                                    <span class="sorting-indicator"></span>
                                                <?php endif; ?>
                                            </a>
                                        </th>
                                        <th scope="col" class="manage-column column-last-booking">
                                            <a href="<?php echo add_query_arg(['orderby' => 'last_booking', 'order' => (isset($_GET['orderby']) && $_GET['orderby'] === 'last_booking' && isset($_GET['order']) && $_GET['order'] === 'ASC') ? 'DESC' : 'ASC']); ?>">
                                                <?php _e('Last Booking', 'vandel-booking'); ?>
                                                <?php if (isset($_GET['orderby']) && $_GET['orderby'] === 'last_booking'): ?>
                                                    <span class="sorting-indicator"></span>
                                                <?php endif; ?>
                                            </a>
                                        </th>
                            ...
                            <td class="column-bookings">
                                <?php echo number_format_i18n(isset($client->bookings_count) ? $client->bookings_count : 0); ?>
                            </td>
                            <td class="column-total-spent">
                                <?php echo \VandelBooking\Helpers::formatPrice(isset($client->total_spent) ? $client->total_spent : 0); ?>
                            </td>
                            <td class="column-last-booking">
                                <?php
                                if (!empty($client->last_booking)) {
                                    echo date_i18n(get_option('date_format'), strtotime($client->last_booking));
                                } else {
                                    echo '—';
                                }
                                ?>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <th scope="col" class="manage-column column-phone"><?php _e('Phone', 'vandel-booking'); ?></th>
                                        <th scope="col" class="manage-column column-bookings"><?php _e('Bookings', 'vandel-booking'); ?></th>
                                        <th scope="col" class="manage-column column-total-spent"><?php _e('Total Spent', 'vandel-booking'); ?></th>
                                        <th scope="col" class="manage-column column-last-booking"><?php _e('Last Booking', 'vandel-booking'); ?></th>
                                        <th scope="col" class="manage-column column-actions"><?php _e('Actions', 'vandel-booking'); ?></th>
                                    </tr>
                                </tfoot>
                            </table>
     ...
     */
    private function renderAddClientForm() {
    ...
    /**
     * Process bulk client actions
     */
    private function processBulkClientActions() {
        // Verify nonce
        ...
        switch ($action) {
            case 'delete':
                $this->bulkDeleteClients($client_ids);
                break;
                
            case 'export':
                $this->bulkExportClients($client_ids);
                break;
        }
    }
     ...
     */
    private function bulkDeleteClients($client_ids) {
        if (!class_exists('\\VandelBooking\\Client\\ClientModel')) {
     ...
     * @param array $client_ids Array of client IDs
     */
    private function bulkExportClients($client_ids) {
        if (!class_exists('\\VandelBooking\\Client\\ClientModel')) {
            wp_redirect(add_query_arg([
                'page' => 'vandel-dashboard', 
                'tab' => 'clients', 
    ...
    /**
     * Display clients status messages
     */
    private function displayClientsStatusMessage() {
...
/**
     ...
     */
    private function renderClientImportPage() {
     ...
     */
    public function handleClientImport() {
        // Verify nonce
        if (!isset($_POST['vandel_import_nonce']) || !wp_verify_nonce($_POST['vandel_import_nonce'], 'vandel_import_clients')) {
        ...
        // Check file type
        $file_info = pathinfo($_FILES['clients_csv']['name']);
        ...
        // Read CSV file
        $csv_data = file_get_contents($_FILES['clients_csv']['tmp_name']);
        
        // Process CSV data if we have ClientModel
        if (class_exists('\\VandelBooking\\Client\\ClientModel')) {
            $client_model = new \VandelBooking\Client\ClientModel();
            
            try {
                $import_results = $client_model->importFromCSV($csv_data);
                
                // Store results in a transient for the next page load
                set_transient('vandel_client_import_results', $import_results, 60); // 1 minute expiration
                
                // Redirect to import page to display results
                wp_redirect(admin_url('admin.php?page=vandel-dashboard&tab=clients&action=import'));
                exit;
