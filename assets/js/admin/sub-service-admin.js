...
(function ($) {
  "use strict";
  console.log("Sub-Service Admin JS loaded");
  ...
  function initInputTypeToggle() {
    ...
    $inputTypeRadios.on("change", function () {
      ...
      // Additional handling for other input types
      updateInputTypeVisibility(selectedType);
    });

    // Trigger initial state on page load
    const initialType = $inputTypeRadios.filter(":checked").val();
    updateInputTypeVisibility(initialType);
  }

  /**
   * Update visibility of input type specific sections
   *
   * @param {string} selectedType Currently selected input type
   */
  function updateInputTypeVisibility(selectedType) {
    const $optionsContainer = $("#vandel-options-container");
    const $numberConfig = $("#vandel-number-config");
    ...
    // Show/hide number configuration
    if (selectedType === "number") {
      $numberConfig.slideDown(200);
    } else {
      $numberConfig.slideUp(200);
